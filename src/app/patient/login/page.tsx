...
export default function PatientLoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      setTimeout(() => {
        authApi.googleSignup({ name: 'Krishna Kumar', email: 'krishna@gmail.com', avatar: '', googleId: 'google_' + Date.now() }, portalConfig.id)
          .then((r) => { if (r.success) { setUser(r.data.user); toast({ title: 'Login successful!' }); setTimeout(() => router.push(portalConfig.dashboardRoute), 1500); } else { toast({ variant: 'destructive', title: 'Error', description: r.error || 'Failed' }); } })
          .finally(() => setProcessing(false));
      }, 1500);
    } catch { toast({ variant: 'destructive', title: 'Error', description: 'Failed to login with Google' }); setProcessing(false); }
  };
  ...
  return (
    <div className="min-h-screen flex bg-background dark:bg-[#1a1c1b]">
      ...
      <div className="hidden lg:flex w-1/2 relative overflow-hidden">
        <Image src="/images/stitch/patient-login-bg.jpg" alt="Healthcare" fill className="object-cover" priority />
        <div className="absolute inset-0 bg-gradient-to-t from-primary/70 via-primary/30 to-transparent" />
        <div className="absolute bottom-12 left-12 right-12 text-on-primary z-10">
          <h2 className="font-newsreader text-h2 mb-4">Your Health, Your Journey.</h2>
          <p className="font-inter text-body-lg opacity-90">Access your records, connect with doctors, and take control of your wellness.</p>
        </div>
      </div>
      {/* Right — Form */}
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-8 lg:p-16">
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 text-primary dark:text-primary-fixed-dim mb-6">
              <span className="material-symbols-outlined text-[32px]">spa</span>
              <span className="font-newsreader text-2xl font-bold text-primary-container dark:text-on-primary-container">Ayu-Raksha</span>
            </div>
            <h1 className="font-newsreader text-h3 text-on-background dark:text-surface-container-lowest mb-2">Patient Portal Login</h1>
            <p className="font-inter text-body-sm text-outline dark:text-outline-variant">Access your health records securely.</p>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                ...
                <input id="email" type="email" placeholder="patient@example.com" {...register('email')} disabled={isProcessing} autoComplete="email" className={inputCls} />
              </div>
            ...
            <button type="submit" disabled={isProcessing} className="w-full bg-primary-container dark:bg-primary-fixed-dim text-on-primary dark:text-on-primary-fixed py-3 rounded-lg font-button text-button hover:bg-on-primary-fixed-variant transition-colors shadow-[0_4px_12px_rgba(52,92,79,0.15)] active:scale-[0.98] disabled:opacity-50">Sign In</button>
          </form>
          <div className="relative my-6"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-outline-variant/30" /></div><div className="relative flex justify-center"><span className="bg-background dark:bg-[#1a1c1b] px-3 font-inter text-body-sm text-outline">or continue with</span></div></div>
          ...
          <div className="mt-8 text-center space-y-3">
            <p className="text-body-sm text-outline">Don&apos;t have an account? <button onClick={() => router.push(portalConfig.signupRoute)} className="text-primary-container font-medium hover:underline">Create Account</button></p>
          ...
          <div className="mt-6 bg-secondary-container/30 border border-secondary/20 p-4 rounded-lg text-center">
            <p className="font-label-bold text-label-bold text-on-secondary-container mb-2">DEMO CREDENTIALS</p>
            <p className="text-body-sm text-on-surface-variant"><span className="font-medium">Email:</span> hritik.srivastava15@gmail.com</p>
            <p className="text-body-sm text-on-surface-variant"><span className="font-medium">Password:</span> 12345678</p>
          </div>
        </div>
      </div>
    </div>
